...
CREATE TABLE IF NOT EXISTS `clinica`.`vacinacao` (
  ...
  `vacina_Id_frasco` INT NOT NULL,
  `carteira_animal_idanimal` INT NOT NULL AUTO_INCREMENT,
  ...
  PRIMARY KEY (`carteira_animal_idanimal`, `animal_idanimal`),
  INDEX `fk_vacinacao_veterinario1_idx` (`veterinario_crmv` ASC) VISIBLE,
  INDEX `fk_vacinacao_vacina1_idx` (`vacina_id_vacina` ASC) VISIBLE,
  UNIQUE INDEX `carteira_animal_idanimal_UNIQUE` (`carteira_animal_idanimal` ASC) VISIBLE,
  INDEX `fk_vacinacao_animal1_idx` (`animal_idanimal` ASC) VISIBLE,
